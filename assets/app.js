...
{
  "meta": {
    "pageTitle": "Nuestra Boda",
    "ogTitle": "Nuestra Boda",
    "ogDescription": "Nos encantará compartir contigo este día tan especial."
  },
  "event": {
    "label": "Nuestra Boda",
  ...
  "story": {
    ...
    "paragraph1": "Hay momentos que se vuelven eternos cuando se comparten con las personas que amamos. Hoy queremos celebrar el camino que nos trajo hasta aquí y la alegría de comenzar una nueva etapa juntos.",
    "paragraph2": "Gracias por acompañarnos con tu cariño, tus buenos deseos y tu presencia en un día que guardaremos para siempre en el corazón.",
    "showImage": true
  },
  "family": {
  ...
  "countdown": {
    ...
    "intro": "La cuenta regresiva ha comenzado para celebrar nuestro gran día."
  },
  "dressCode": {
  ...
  "gifts": {
    ...
    "intro": "Tu presencia es nuestro regalo más valioso. Si deseas tener un detalle con nosotros, aquí encontrarás algunas opciones.",
    "option1Title": "Lluvia de sobres",
    "option1Copy": "Si lo prefieres, podrás acompañarnos con un sobre el día del evento. Lo recibiremos con mucho cariño y gratitud.",
    "showOption1": true,
    "option2Title": "Regalo para el hogar",
    "option2Copy": "También puedes elegir un detalle especial para esta nueva etapa que comenzamos juntos.",
    "showOption2": true,
  ...
  "album": {
    "title": "Álbum digital",
    "copy": "Comparte las mejores fotos de nuestra boda y ayúdanos a conservar los recuerdos más especiales.",
  ...
  "style": {
    ...
    "titleColor": "#735326",
    "subtitleColor": "#735326",
    "paragraphColor": "#34291f",
    "iconColor": "#b38a4c",
    "buttonBg": "#b38a4c",
    "buttonText": "#fffaf2",
    "buttonBorder": "#b38a4c",
    "buttonRadius": 999,
    "buttonShadow": true,
    "secondaryButtonBg": "#fffaf2",
    "secondaryButtonText": "#34291f",
    "sectionBg": "#fffdf8",
    "sectionBorder": "#d8b36a",
    "sectionRadius": 32,
    "sectionOpacity": 94,
    "cardBg": "#fffaf2",
    "cardBorder": "#d8b36a",
  ...
  "closing": {
    "note": "Gracias por acompañarnos en el inicio de esta nueva historia"
  },
  "brand": {
  ...
  "sectionColors": {
    ...
    const editorLayout = editorPanel.querySelector(".editor-layout");
    ...
    const SECTION_COLOR_FIELDS = {
      story: "cfgSectionColorStory",
      parents: "cfgSectionColorParents",
      godparents: "cfgSectionColorGodparents",
      ceremony: "cfgSectionColorCeremony",
      reception: "cfgSectionColorReception",
      timeline: "cfgSectionColorTimeline",
      countdown: "cfgSectionColorCountdown",
      gallery: "cfgSectionColorGallery",
      dress: "cfgSectionColorDress",
      rsvp: "cfgSectionColorRsvp",
      gifts: "cfgSectionColorGifts",
      agenda: "cfgSectionColorAgenda",
      album: "cfgSectionColorAlbum"
    };
    ...
    function styleValue(key, fallback = "") {
      const style = INVITATION_CONFIG.style || {};
      const defaults = DEFAULT_CONFIG.style || {};
      return style[key] ?? defaults[key] ?? fallback;
    }

    function numberStyleValue(key, fallback, min = -Infinity, max = Infinity) {
      const value = Number(styleValue(key, fallback));
      const resolved = Number.isFinite(value) ? value : fallback;
      return Math.max(min, Math.min(max, resolved));
    }

    function colorStyleValue(key, fallback) {
      return normalizeHexColor(styleValue(key, fallback), fallback);
    }

    function alphaFromPercent(value) {
    ...
    function setEditorChecked(id, value) {
      const field = getEditorField(id);
    ...
    function sanitizeStyleConfig() {
      INVITATION_CONFIG.style = deepMerge(DEFAULT_CONFIG.style || {}, INVITATION_CONFIG.style || {});
      INVITATION_CONFIG.music = deepMerge(DEFAULT_CONFIG.music || {}, INVITATION_CONFIG.music || {});
      INVITATION_CONFIG.social = deepMerge(DEFAULT_CONFIG.social || {}, INVITATION_CONFIG.social || {});
      INVITATION_CONFIG.sectionColors = deepMerge(DEFAULT_CONFIG.sectionColors || {}, INVITATION_CONFIG.sectionColors || {});
    }
    ...
    function getEventCopy() {
      ...
      if (!label && !name) {
        return {
          ...
          calendarDetails: "Nos encantará compartir contigo este día tan especial."
        };
      }

      if (/boda|wedding|nuestra boda|matrimonio/.test(normalized)) {
        return {
          pageTitle: name ? `${label} | ${name}` : label,
          celebrationText: name ? `la boda de ${name}` : "nuestra boda",
          eventHeader: name ? `${label} de ${name}` : label,
          calendarTitle: name ? `Boda de ${name}` : label,
          calendarDetails: name
            ? `Nos encantará compartir contigo la boda de ${name}.`
            : "Nos encantará compartir contigo nuestra boda."
        };
      }

      const joinedTitle = label && name ? `${label} | ${name}` : (label || name);
      return {
        ...
        calendarDetails: joinedTitle
          ? `Nos encantará compartir contigo ${joinedTitle}.`
    ...
    function applyDesignSettings() {
      const titleColor = colorStyleValue("titleColor", "#735326");
      const subtitleColor = colorStyleValue("subtitleColor", "#735326");
      const paragraphColor = colorStyleValue("paragraphColor", INVITATION_CONFIG.theme.text || "#34291f");
      const sectionBg = colorStyleValue("sectionBg", "#fffdf8");
      const cardBg = colorStyleValue("cardBg", "#fffaf2");
      const sectionOpacity = alphaFromPercent(numberStyleValue("sectionOpacity", 94, 35, 100));
      const cardOpacity = alphaFromPercent(numberStyleValue("cardOpacity", 92, 35, 100));
      const shadowOpacity = alphaFromPercent(numberStyleValue("shadowOpacity", 15, 0, 60));
      const transitionMs = numberStyleValue("transitionDuration", 900, 0, 3000);
      const animationSpeed = numberStyleValue("animationSpeed", 1, 0.2, 3);
      const gradientStrength = numberStyleValue("sectionGradientStrength", 1, 0, 1);
      const animationsEnabled = INVITATION_CONFIG.style.animationsEnabled !== false;
      const gradientsEnabled = INVITATION_CONFIG.style.gradientEnabled !== false;
      const backgroundImage = String(styleValue("backgroundImage", "") || "").trim();

      setRootVar("--font-body", cssFontValue(styleValue("contentFont", INVITATION_CONFIG.theme.bodyFont), "\"Cormorant Garamond\", serif"));
      setRootVar("--font-title", cssFontValue(styleValue("titleFont", INVITATION_CONFIG.theme.bodyFont), "\"Cormorant Garamond\", serif"));
      setRootVar("--title-color", titleColor);
      setRootVar("--subtitle-color", subtitleColor);
      setRootVar("--paragraph-color", paragraphColor);
      setRootVar("--icon-color", colorStyleValue("iconColor", INVITATION_CONFIG.theme.gold500 || "#b38a4c"));
      setRootVar("--button-bg", colorStyleValue("buttonBg", "#b38a4c"));
      setRootVar("--button-text", colorStyleValue("buttonText", "#fffaf2"));
      setRootVar("--button-border", colorStyleValue("buttonBorder", "#b38a4c"));
      setRootVar("--button-radius", `${numberStyleValue("buttonRadius", 999, 0, 999)}px`);
      setRootVar("--button-shadow", styleValue("buttonShadow", true) ? `0 12px 24px rgba(${rgbList(colorStyleValue("shadowColor", "#5c4123"))}, ${Math.max(0.08, shadowOpacity)})` : "none");
      setRootVar("--secondary-button-bg", colorStyleValue("secondaryButtonBg", "#fffaf2"));
      setRootVar("--secondary-button-text", colorStyleValue("secondaryButtonText", paragraphColor));
      setRootVar("--section-bg", rgbaColor(sectionBg, sectionOpacity));
      setRootVar("--section-bg-soft", gradientsEnabled ? rgbaColor(sectionBg, Math.max(0.35, sectionOpacity - (0.08 * gradientStrength))) : rgbaColor(sectionBg, sectionOpacity));
      setRootVar("--section-border", rgbaColor(colorStyleValue("sectionBorder", "#d8b36a"), 0.34));
      setRootVar("--section-radius", `${numberStyleValue("sectionRadius", 32, 0, 60)}px`);
      setRootVar("--card-bg", rgbaColor(cardBg, cardOpacity));
      setRootVar("--card-bg-soft", gradientsEnabled ? rgbaColor(cardBg, Math.max(0.3, cardOpacity - (0.08 * gradientStrength))) : rgbaColor(cardBg, cardOpacity));
      setRootVar("--card-border", rgbaColor(colorStyleValue("cardBorder", "#d8b36a"), 0.3));
      setRootVar("--card-radius", `${numberStyleValue("cardRadius", 22, 0, 48)}px`);
      setRootVar("--shadow-color-rgb", rgbList(colorStyleValue("shadowColor", "#5c4123")));
      setRootVar("--shadow-opacity", shadowOpacity.toFixed(2));
      setRootVar("--hero-name-size", `${numberStyleValue("heroNameSize", 62, 34, 110)}px`);
      setRootVar("--section-title-size", `${numberStyleValue("sectionTitleSize", 32, 20, 64)}px`);
      setRootVar("--body-text-size", `${numberStyleValue("bodyTextSize", 18, 12, 28)}px`);
      setRootVar("--script-text-size", `${numberStyleValue("scriptTextSize", 37, 24, 72)}px`);
      setRootVar("--button-text-size", `${numberStyleValue("buttonTextSize", 15, 11, 24)}px`);
      setRootVar("--countdown-number-size", `${numberStyleValue("countdownNumberSize", 30, 18, 54)}px`);
      setRootVar("--section-padding-y", `${numberStyleValue("sectionPaddingY", 30, 12, 80)}px`);
      setRootVar("--section-padding-x", `${numberStyleValue("sectionPaddingX", 22, 10, 60)}px`);
      setRootVar("--section-gap", `${numberStyleValue("sectionGap", 18, 0, 60)}px`);
      setRootVar("--card-padding", `${numberStyleValue("cardPadding", 18, 8, 50)}px`);
      setRootVar("--separator-color-rgb", rgbList(colorStyleValue("separatorColor", "#b38a4c")));
      setRootVar("--separator-width", `${numberStyleValue("separatorWidth", 138, 40, 260)}px`);
      setRootVar("--separator-margin", `${numberStyleValue("separatorMargin", 26, 0, 80)}px`);
      setRootVar("--transition-duration", `${transitionMs}ms`);
      setRootVar("--animation-speed", animationSpeed);
      setRootVar("--gradient-strength", gradientsEnabled ? gradientStrength : 0);
      setRootVar("--page-bg-image", backgroundImage ? `url("../${extractFileName(backgroundImage, backgroundImage)}")` : "none");
      setRootVar("--page-bg-overlay", backgroundImage ? "linear-gradient(rgba(255, 250, 242, 0.76), rgba(244, 234, 220, 0.82))" : "transparent");
      setRootVar("--menu-bg", colorStyleValue("menuBg", "#fffaf2"));
      setRootVar("--footer-bg", colorStyleValue("footerBg", "#f1e5d4"));
      setRootVar("--footer-text", colorStyleValue("footerText", paragraphColor));
      setRootVar("--floating-bg", colorStyleValue("floatingBg", "#b38a4c"));
      setRootVar("--floating-text", colorStyleValue("floatingText", "#fffaf2"));
      document.documentElement.classList.toggle("animations-off", !animationsEnabled);

      if (audio) {
        const src = extractFileName(INVITATION_CONFIG.music?.src || "", "cancio.mp3");
    ...
    function renderSocialLinks() {
      ...
      const items = [
        ["Instagram", INVITATION_CONFIG.social?.instagram],
        ["Facebook", INVITATION_CONFIG.social?.facebook],
        ["TikTok", INVITATION_CONFIG.social?.tiktok],
        ["Web", INVITATION_CONFIG.social?.website]
      ].filter(([, href]) => String(href || "").trim());

      socialLinks.innerHTML = "";
      socialLinks.hidden = !items.length;
    ...
    function applySectionColorOverrides() {
      const sectionMap = getSectionMap();
      const opacity = alphaFromPercent(numberStyleValue("sectionOpacity", 94, 35, 100));
      const gradientsEnabled = INVITATION_CONFIG.style.gradientEnabled !== false;
      const gradientStrength = numberStyleValue("sectionGradientStrength", 1, 0, 1);
      Object.entries(SECTION_COLOR_FIELDS).forEach(([key]) => {
        const element = sectionMap[key];
        if (!element) return;
        const fallback = colorStyleValue("sectionBg", "#fffdf8");
        const color = normalizeHexColor(INVITATION_CONFIG.sectionColors?.[key], fallback);
        element.style.setProperty("--section-bg", rgbaColor(color, opacity));
        element.style.setProperty("--section-bg-soft", gradientsEnabled ? rgbaColor(color, Math.max(0.35, opacity - (0.08 * gradientStrength))) : rgbaColor(color, opacity));
      });
    }
    ...
    function updateEditorNavState() {
      ...
      const scrollContainer = window.matchMedia("(max-width: 720px)").matches && editorLayout ? editorLayout : editorContent;
      const active = sections.find((section) => section.offsetTop - scrollContainer.scrollTop > -80)
    ...
    function applyInvitationConfig() {
      ...
      setRootVar("--font-body", cssFontValue(INVITATION_CONFIG.style?.contentFont || INVITATION_CONFIG.theme.bodyFont, "\"Cormorant Garamond\", serif"));
    ...
    function typeText(element, text, speed = 20) {
      return new Promise((resolve) => {
        if (INVITATION_CONFIG.style?.animationsEnabled === false) {
    ...
    async function typeStoryParagraphs() {
      const storySpeed = Math.max(4, Math.round(8 / numberStyleValue("animationSpeed", 1, 0.2, 3)));
      for (const paragraph of storyParagraphs) {
        const fullText = paragraph.dataset.fulltext || "";
        await typeText(paragraph, fullText, storySpeed);
        await new Promise((resolve) => setTimeout(resolve, Math.max(80, 180 / numberStyleValue("animationSpeed", 1, 0.2, 3))));
      }
    }
    ...
    function fillEditorPanel() {
      ...
      setEditorValue("cfgTitleFont", styleValue("titleFont", "Cormorant Garamond"));
      setEditorValue("cfgContentFont", styleValue("contentFont", "Cormorant Garamond"));
      setEditorValue("cfgTitleColor", colorStyleValue("titleColor", "#735326"));
      setEditorValue("cfgSubtitleColor", colorStyleValue("subtitleColor", "#735326"));
      setEditorValue("cfgParagraphColor", colorStyleValue("paragraphColor", "#34291f"));
      setEditorValue("cfgIconColor", colorStyleValue("iconColor", "#b38a4c"));
      setEditorValue("cfgButtonBg", colorStyleValue("buttonBg", "#b38a4c"));
      setEditorValue("cfgButtonText", colorStyleValue("buttonText", "#fffaf2"));
      setEditorValue("cfgButtonBorder", colorStyleValue("buttonBorder", "#b38a4c"));
      setEditorValue("cfgButtonRadius", numberStyleValue("buttonRadius", 999, 0, 999));
      setEditorValue("cfgSecondaryButtonBg", colorStyleValue("secondaryButtonBg", "#fffaf2"));
      setEditorValue("cfgSecondaryButtonText", colorStyleValue("secondaryButtonText", "#34291f"));
      setEditorValue("cfgSectionBg", colorStyleValue("sectionBg", "#fffdf8"));
      setEditorValue("cfgSectionBorder", colorStyleValue("sectionBorder", "#d8b36a"));
      setEditorValue("cfgSectionRadius", numberStyleValue("sectionRadius", 32, 0, 60));
      setEditorValue("cfgSectionOpacity", numberStyleValue("sectionOpacity", 94, 35, 100));
      setEditorValue("cfgCardBg", colorStyleValue("cardBg", "#fffaf2"));
      setEditorValue("cfgCardBorder", colorStyleValue("cardBorder", "#d8b36a"));
      setEditorValue("cfgCardRadius", numberStyleValue("cardRadius", 22, 0, 48));
      setEditorValue("cfgCardOpacity", numberStyleValue("cardOpacity", 92, 35, 100));
      setEditorValue("cfgShadowColor", colorStyleValue("shadowColor", "#5c4123"));
      setEditorValue("cfgShadowOpacity", numberStyleValue("shadowOpacity", 15, 0, 60));
      Object.entries(SECTION_COLOR_FIELDS).forEach(([key, id]) => {
        setEditorValue(id, normalizeHexColor(INVITATION_CONFIG.sectionColors?.[key], colorStyleValue("sectionBg", "#fffdf8")));
      });
      setEditorValue("cfgHeroNameSize", numberStyleValue("heroNameSize", 62, 34, 110));
      setEditorValue("cfgSectionTitleSize", numberStyleValue("sectionTitleSize", 32, 20, 64));
      setEditorValue("cfgBodyTextSize", numberStyleValue("bodyTextSize", 18, 12, 28));
      setEditorValue("cfgScriptTextSize", numberStyleValue("scriptTextSize", 37, 24, 72));
      setEditorValue("cfgButtonTextSize", numberStyleValue("buttonTextSize", 15, 11, 24));
      setEditorValue("cfgCountdownNumberSize", numberStyleValue("countdownNumberSize", 30, 18, 54));
      setEditorValue("cfgSectionPaddingY", numberStyleValue("sectionPaddingY", 30, 12, 80));
      setEditorValue("cfgSectionPaddingX", numberStyleValue("sectionPaddingX", 22, 10, 60));
      setEditorValue("cfgSectionGap", numberStyleValue("sectionGap", 18, 0, 60));
      setEditorValue("cfgCardPadding", numberStyleValue("cardPadding", 18, 8, 50));
      setEditorValue("cfgSeparatorColor", colorStyleValue("separatorColor", "#b38a4c"));
      setEditorValue("cfgSeparatorWidth", numberStyleValue("separatorWidth", 138, 40, 260));
      setEditorValue("cfgSeparatorMargin", numberStyleValue("separatorMargin", 26, 0, 80));
      setEditorValue("cfgTransitionDuration", numberStyleValue("transitionDuration", 900, 0, 3000));
      setEditorValue("cfgAnimationSpeed", numberStyleValue("animationSpeed", 1, 0.2, 3));
      setEditorValue("cfgGradientStrength", numberStyleValue("sectionGradientStrength", 1, 0, 1));
      setEditorChecked("cfgAnimationsEnabled", styleValue("animationsEnabled", true));
      setEditorChecked("cfgGradientEnabled", styleValue("gradientEnabled", true));
      setEditorValue("cfgMusicSrc", INVITATION_CONFIG.music?.src || "cancio.mp3");
      setEditorChecked("cfgMusicEnabled", INVITATION_CONFIG.music?.enabled !== false);
      setEditorValue("cfgBackgroundImage", styleValue("backgroundImage", ""));
      setEditorValue("cfgMenuBg", colorStyleValue("menuBg", "#fffaf2"));
      setEditorValue("cfgFooterBg", colorStyleValue("footerBg", "#f1e5d4"));
      setEditorValue("cfgFooterText", colorStyleValue("footerText", "#34291f"));
      setEditorValue("cfgFloatingBg", colorStyleValue("floatingBg", "#b38a4c"));
      setEditorValue("cfgFloatingText", colorStyleValue("floatingText", "#fffaf2"));
      setEditorValue("cfgSocialInstagram", INVITATION_CONFIG.social?.instagram || "");
      setEditorValue("cfgSocialFacebook", INVITATION_CONFIG.social?.facebook || "");
      setEditorValue("cfgSocialTiktok", INVITATION_CONFIG.social?.tiktok || "");
      setEditorValue("cfgSocialWebsite", INVITATION_CONFIG.social?.website || "");
    ...
    function readEditorPanel() {
      ...
      INVITATION_CONFIG.style.titleFont = readEditorValue("cfgTitleFont");
      INVITATION_CONFIG.style.contentFont = readEditorValue("cfgContentFont");
      INVITATION_CONFIG.style.titleColor = readEditorValue("cfgTitleColor");
      INVITATION_CONFIG.style.subtitleColor = readEditorValue("cfgSubtitleColor");
      INVITATION_CONFIG.style.paragraphColor = readEditorValue("cfgParagraphColor");
      INVITATION_CONFIG.style.iconColor = readEditorValue("cfgIconColor");
      INVITATION_CONFIG.style.buttonBg = readEditorValue("cfgButtonBg");
      INVITATION_CONFIG.style.buttonText = readEditorValue("cfgButtonText");
      INVITATION_CONFIG.style.buttonBorder = readEditorValue("cfgButtonBorder");
      INVITATION_CONFIG.style.buttonRadius = Number(readEditorValue("cfgButtonRadius")) || DEFAULT_CONFIG.style.buttonRadius;
      INVITATION_CONFIG.style.secondaryButtonBg = readEditorValue("cfgSecondaryButtonBg");
      INVITATION_CONFIG.style.secondaryButtonText = readEditorValue("cfgSecondaryButtonText");
      INVITATION_CONFIG.style.sectionBg = readEditorValue("cfgSectionBg");
      INVITATION_CONFIG.style.sectionBorder = readEditorValue("cfgSectionBorder");
      INVITATION_CONFIG.style.sectionRadius = Number(readEditorValue("cfgSectionRadius")) || DEFAULT_CONFIG.style.sectionRadius;
      INVITATION_CONFIG.style.sectionOpacity = Number(readEditorValue("cfgSectionOpacity")) || DEFAULT_CONFIG.style.sectionOpacity;
      INVITATION_CONFIG.style.cardBg = readEditorValue("cfgCardBg");
      INVITATION_CONFIG.style.cardBorder = readEditorValue("cfgCardBorder");
      INVITATION_CONFIG.style.cardRadius = Number(readEditorValue("cfgCardRadius")) || DEFAULT_CONFIG.style.cardRadius;
      INVITATION_CONFIG.style.cardOpacity = Number(readEditorValue("cfgCardOpacity")) || DEFAULT_CONFIG.style.cardOpacity;
      INVITATION_CONFIG.style.shadowColor = readEditorValue("cfgShadowColor");
      INVITATION_CONFIG.style.shadowOpacity = Number(readEditorValue("cfgShadowOpacity"));
      Object.entries(SECTION_COLOR_FIELDS).forEach(([key, id]) => {
        INVITATION_CONFIG.sectionColors[key] = readEditorValue(id);
      });
      INVITATION_CONFIG.style.heroNameSize = Number(readEditorValue("cfgHeroNameSize")) || DEFAULT_CONFIG.style.heroNameSize;
      INVITATION_CONFIG.style.sectionTitleSize = Number(readEditorValue("cfgSectionTitleSize")) || DEFAULT_CONFIG.style.sectionTitleSize;
      INVITATION_CONFIG.style.bodyTextSize = Number(readEditorValue("cfgBodyTextSize")) || DEFAULT_CONFIG.style.bodyTextSize;
      INVITATION_CONFIG.style.scriptTextSize = Number(readEditorValue("cfgScriptTextSize")) || DEFAULT_CONFIG.style.scriptTextSize;
      INVITATION_CONFIG.style.buttonTextSize = Number(readEditorValue("cfgButtonTextSize")) || DEFAULT_CONFIG.style.buttonTextSize;
      INVITATION_CONFIG.style.countdownNumberSize = Number(readEditorValue("cfgCountdownNumberSize")) || DEFAULT_CONFIG.style.countdownNumberSize;
      INVITATION_CONFIG.style.sectionPaddingY = Number(readEditorValue("cfgSectionPaddingY")) || DEFAULT_CONFIG.style.sectionPaddingY;
      INVITATION_CONFIG.style.sectionPaddingX = Number(readEditorValue("cfgSectionPaddingX")) || DEFAULT_CONFIG.style.sectionPaddingX;
      INVITATION_CONFIG.style.sectionGap = Number(readEditorValue("cfgSectionGap"));
      INVITATION_CONFIG.style.cardPadding = Number(readEditorValue("cfgCardPadding")) || DEFAULT_CONFIG.style.cardPadding;
      INVITATION_CONFIG.style.separatorColor = readEditorValue("cfgSeparatorColor");
      INVITATION_CONFIG.style.separatorWidth = Number(readEditorValue("cfgSeparatorWidth")) || DEFAULT_CONFIG.style.separatorWidth;
      INVITATION_CONFIG.style.separatorMargin = Number(readEditorValue("cfgSeparatorMargin"));
      INVITATION_CONFIG.style.transitionDuration = Number(readEditorValue("cfgTransitionDuration"));
      INVITATION_CONFIG.style.animationSpeed = Number(readEditorValue("cfgAnimationSpeed")) || DEFAULT_CONFIG.style.animationSpeed;
      INVITATION_CONFIG.style.sectionGradientStrength = Number(readEditorValue("cfgGradientStrength"));
      INVITATION_CONFIG.style.animationsEnabled = getEditorField("cfgAnimationsEnabled").checked;
      INVITATION_CONFIG.style.gradientEnabled = getEditorField("cfgGradientEnabled").checked;
      INVITATION_CONFIG.music.src = extractFileName(readEditorTrimmedValue("cfgMusicSrc"), "cancio.mp3");
      INVITATION_CONFIG.music.enabled = getEditorField("cfgMusicEnabled").checked;
      INVITATION_CONFIG.style.backgroundImage = readEditorTrimmedValue("cfgBackgroundImage");
      INVITATION_CONFIG.style.menuBg = readEditorValue("cfgMenuBg");
      INVITATION_CONFIG.style.footerBg = readEditorValue("cfgFooterBg");
      INVITATION_CONFIG.style.footerText = readEditorValue("cfgFooterText");
      INVITATION_CONFIG.style.floatingBg = readEditorValue("cfgFloatingBg");
      INVITATION_CONFIG.style.floatingText = readEditorValue("cfgFloatingText");
      INVITATION_CONFIG.social.instagram = readEditorTrimmedValue("cfgSocialInstagram");
      INVITATION_CONFIG.social.facebook = readEditorTrimmedValue("cfgSocialFacebook");
      INVITATION_CONFIG.social.tiktok = readEditorTrimmedValue("cfgSocialTiktok");
      INVITATION_CONFIG.social.website = readEditorTrimmedValue("cfgSocialWebsite");
    ...
    function fadeInAudio() {
      if (INVITATION_CONFIG.music?.enabled === false) return;
      audio.volume = 0;
      audio.play().then(() => {
        let volume = 0;
    ...
    openBtn.addEventListener("click", () => {
      ...
      if (INVITATION_CONFIG.music?.enabled === false) {
        musicBtn.classList.remove("show");
        audio.pause();
      } else {
    ...
    editorPanel.querySelectorAll("[data-editor-jump]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        const useLayoutScroll = window.matchMedia("(max-width: 720px)").matches && editorLayout;
        const scrollContainer = useLayoutScroll ? editorLayout : editorContent;
        const offset = useLayoutScroll ? Math.max(0, target.offsetTop - 12) : Math.max(0, target.offsetTop - 8);
    ...
    if (editorLayout) {
      editorLayout.addEventListener("scroll", updateEditorNavState);
    }
